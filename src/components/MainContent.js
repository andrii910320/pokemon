...
export default function MainContent(props) {
    ...
    console.log(listAttributePokemon)
    console.log(listAttributePokemonSecond)
    ...
    function handlePageState(event) {
        ...
        if (id === 'next') {
            setPage(prevState => {
                if (prevState * numberCards >= actualRenderList.length - 1) return prevState
                return prevState + 1
            })
        }
    }
    ...
    React.useEffect(() => {
        setActualRenderList(() => {
            setPage(1)
            let findBySearch = props.searchPokemon ? pokemonList.filter((pokemon) => pokemon.name.includes(props.searchPokemon)) : pokemonList
    ...
    const startIndex = (page - 1) * Number(parametersView.numberCards)
    const endIndex = startIndex + Number(parametersView.numberCards)
    const sliceActualRenderList =
        actualRenderList.length > parametersView.numberCards ?
